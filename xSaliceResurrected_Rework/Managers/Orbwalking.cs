namespace xSaliceResurrected_Rework
{
    ...
    using static LeagueSharp.ObjectManager;
    ...
    public static class Orbwalking
    {
        ...
        static Orbwalking()
        {
            ...
            {
                AzirSoliders = Get<Obj_AI_Minion>().Where(x => x.IsAlly && x.Name == "AzirSoldier" && x.HasBuff("azirwspawnsound")).ToList();
        ...
        public static bool CanAttack()
        {
            switch (Player.ChampionName)
            {
                case "Graves":
                    ...
                    return Utils.GameTimeTickCount + Game.Ping / 2 + 25 >= LastAATick + attackDelay && Player.HasBuff("GravesBasicAttackAmmo1");
        ...
        public static bool InAutoAttackRange(AttackableUnit target)
        {
            ...
            return Vector2.DistanceSquared(@base?.ServerPosition.To2D() ?? target.Position.To2D(), Player.ServerPosition.To2D()) <= myRange * myRange;
        }
        ...
        private static void OnProcessSpell(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs Spell)
        {
            try
            {
                ...
                if (sender.IsMe && (Spell.Target is Obj_AI_Base || Spell.Target is Obj_BarracksDampener || Spell.Target is Obj_HQ))
                {
        ...
        public class Orbwalker
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                var miscMenu = Menu.AddSubMenu(new Menu("Misc", "Misc"));
                {
                    ...
                    miscMenu.AddItem(new MenuItem("HoldPosRadius", "Hold Position Radius").SetValue(new Slider(50, 0, 250)));
                ...
                {
                    orbwalkerKeyMenu.AddItem(new MenuItem("Flee", "Flee", true).SetValue(new KeyBind('Z', KeyBindType.Press)));
                    orbwalkerKeyMenu.AddItem(new MenuItem("LastHit", "Last hit", true).SetValue(new KeyBind('X', KeyBindType.Press)));
                    orbwalkerKeyMenu.AddItem(new MenuItem("Farm", "Mixed", true).SetValue(new KeyBind('C', KeyBindType.Press)));
                    orbwalkerKeyMenu.AddItem(new MenuItem("Freeze", "Freeze", true).SetValue(new KeyBind('N', KeyBindType.Press)));
                    orbwalkerKeyMenu.AddItem(new MenuItem("LaneClear", "LaneClear", true).SetValue(new KeyBind('V', KeyBindType.Press)));
                    orbwalkerKeyMenu.AddItem(new MenuItem("Orbwalk", "Combo", true).SetValue(new KeyBind(32, KeyBindType.Press)));
                }
            ...
            public OrbwalkingMode ActiveMode
            {
                get
                {
                    ...
                    if (Menu.Item("Freeze", true).GetValue<KeyBind>().Active)
                    {
            ...
            public virtual AttackableUnit GetTarget()
            {
                ...
                #region attack hero target
                if (ObjectManager.Player.ChampionName == "Azir" && (mode == OrbwalkingMode.Combo || mode == OrbwalkingMode.Mixed))
                {
                    ...
                    var bestTarget = HeroManager.Enemies.Where(InAzirAutoAttackRange).OrderByDescending(GetAzirAaSandwarriorDamage).FirstOrDefault();
                    ...
                    var normaltarget = TargetSelector.GetTarget(GetAutoAttackRange(), TargetSelector.DamageType.Magical);
                ...
                if (ObjectManager.Player.ChampionName != "Azir" && (mode == OrbwalkingMode.Mixed || mode == OrbwalkingMode.LaneClear) && !Menu.Item("PriorizeFarm").GetValue<bool>())
                {
                ...
                if (mode == OrbwalkingMode.LaneClear || mode == OrbwalkingMode.Mixed || mode == OrbwalkingMode.LastHit || mode == OrbwalkingMode.Freeze)
                {
                    var MinionList =
                        Get<Obj_AI_Minion>()
                            ...
                            &&
                            (ObjectManager.Player.ChampionName == "Azir" ? InAzirAutoAttackRange(minion) :
                    ...
                    foreach (var minion in MinionList)
                    {
                        ...
                        {
                            if (predHealth <= (InSoldierAttackRange(minion) ? GetAzirAaSandwarriorDamage(minion) - 30 : ObjectManager.Player.GetAutoAttackDamage(minion, true)))
                            {
                                return minion;
                            }
                        }
                ...
                if (mode != OrbwalkingMode.LastHit)
                {
                    if (mode != OrbwalkingMode.LaneClear || !ShouldWait())
                    {
                        ...
                        if (target.IsValidTarget() && player.ChampionName == "Azir" ? InAzirAutoAttackRange(target) : InAutoAttackRange(target))
                        {
                            return target;
                        }
                    }
                ...
                {
                    var jminions = Get<Obj_AI_Minion>().Where(mob => mob.IsValidTarget() && mob.Team == GameObjectTeam.Neutral && 
                    (player.ChampionName == "Azir" ? InAzirAutoAttackRange(mob) : InAutoAttackRange(mob)) &&
                ...
                if (mode == OrbwalkingMode.LaneClear || mode == OrbwalkingMode.Mixed || mode == OrbwalkingMode.LastHit || mode == OrbwalkingMode.Freeze)
                {
                    ...
                    if (closestTower != null && player.Distance(closestTower, true) < 1500 * 1500)
                    {
                        ...
                        var minions = MinionManager.GetMinions(player.Position, player.ChampionName == "Azir" ? 850 : player.AttackRange + 200)
                                .Where(minion => player.ChampionName == "Azir" ? InAzirAutoAttackRange(minion) : InAutoAttackRange(minion) && closestTower.Distance(minion, true) < 900 * 900)
                                .OrderByDescending(minion => minion.CharData.BaseSkinName.Contains("Siege"))
                                .ThenBy(minion => minion.CharData.BaseSkinName.Contains("Super"))
                                .ThenByDescending(minion => minion.MaxHealth)
                ...
                if (mode == OrbwalkingMode.LaneClear)
                {
                    if (!ShouldWait())
                    {
                        if (_prevMinion.IsValidTarget() && player.ChampionName == "Azir" ? InAzirAutoAttackRange(_prevMinion) : InAutoAttackRange(_prevMinion))
                        {
                        ...
                        result = Get<Obj_AI_Minion>()
                            .Where(minion =>
                                        ...
                                        player.ChampionName == "Azir" ? InAzirAutoAttackRange(minion) : InAutoAttackRange(minion) &&
            ...
            public double GetAzirAaSandwarriorDamage(AttackableUnit target)
            {
            ...
            public bool InSoldierAttackRange(AttackableUnit target)
            {
            ...
            private float GetAutoAttackRange(Obj_AI_Base source = null, AttackableUnit target = null)
            {
                if (source == null)
            ...
            public bool InAzirAutoAttackRange(AttackableUnit target)
            {
                if (!target.IsValidTarget())
                    return false;
                ...
                if (!(target is Obj_AI_Base))
                    return false;
                return InSoldierAttackRange(target);
            }
            ...
            {
                return Get<Obj_AI_Minion>().
                    Any(minion => minion.IsValidTarget() && 
                    minion.Team != GameObjectTeam.Neutral &&
                    InAutoAttackRange(minion) && 
                    MinionManager.IsMinion(minion) && 
                    HealthPrediction.LaneClearHealthPrediction
                    (minion, (int)(player.AttackDelay * 1000 * LaneClearWaitTimeMod), FarmDelay) <=
                   (InSoldierAttackRange(minion) ? GetAzirAaSandwarriorDamage(minion) - 30 : player.GetAutoAttackDamage(minion)));
            }

            private void DrawingOnOnDraw(EventArgs args)
            {
                if (ObjectManager.Player.IsDead || MenuGUI.IsShopOpen || MenuGUI.IsChatOpen || MenuGUI.IsScoreboardOpen)
                {
                    return;
                }
                ...
                {
                    Render.Circle.DrawCircle(player.Position, GetRealAutoAttackRange(null) + 65, Menu.Item("AACircle").GetValue<Circle>().Color, Menu.Item("AALineWidth").GetValue<Slider>().Value);
                }
