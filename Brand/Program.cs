...
namespace Flowers_Brand
{
    class Program
    {
        ...
        public static bool HavePassive(Obj_AI_Hero e) => e.HasBuff("Brandablaze");
        ...
        private static void KillStealLogic()
        {
            try
            {
                foreach(var e in HeroManager.Enemies.Where(e => e.IsValidTarget() && !e.IsZombie && CheckTargetSureCanKill(e)))
                {
                    ...
                    //                {
                    //                    Game.PrintChat("Return True in KillStealR if(e.ServerPosition.Distance(m.ServerPosition) < 150)");

                    //                    if (e.Health < R.GetDamage(e) - 10)
                    //                    {
                    //                        Game.PrintChat("Return True in KillStealR  (e.Health < R.GetDamage(e) - 10)");
                    ...
                    //                    else
                    //                    {
                    //                        Game.PrintChat("Return False in KillStealR  (e.Health < R.GetDamage(e) - 10)");

                    //                    }
                    //                }
                    //                else
                    //                {
                    //                    Game.PrintChat("Return False in KillStealR if(e.ServerPosition.Distance(m.ServerPosition) < 150)");
                    //                }
                    //            }
                    //        }
                }
        ...
        private static void EQInterruptEnemySpells(Obj_AI_Hero sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            try
            {
                if(EQInterrupt)
