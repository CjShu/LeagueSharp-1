namespace Flowers_Karthus
{
    ...
    public static class Program
    {
        ...
        {
            foreach (var enemy in HeroManager.Enemies.Where(h => R.IsReady() && h.IsValidTarget() && GetRDamage(h) > h.Health + h.MagicalShield))
            {
        ...
        private static void KillStealLogic()
        {
            ...
            foreach (var target in HeroManager.Enemies)
            {
                ...
                if (Menu.Item("KillStealQ", true).GetValue<bool>() && Q.IsReady() && target.Health + target.MagicalShield < GetQDamage(target))
                {
                    Q.CastTo(target, true);
                }
            }
